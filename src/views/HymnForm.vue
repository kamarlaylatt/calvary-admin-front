<template>
  <v-container fluid class="pa-6">
    <div class="d-flex justify-space-between align-center mb-6">
      <div>
        <h1 class="text-h4 font-weight-bold mb-2">{{ isEditing ? 'Edit Hymn' : 'Add New Hymn' }}</h1>
        <p class="text-subtitle-1 text-medium-emphasis">{{ isEditing ? 'Update hymn details' : 'Create a new hymn' }}</p>
      </div>
      <v-btn variant="outlined" prepend-icon="mdi-arrow-left" @click="$router.push({ path: '/hymns', query: { page: route.query.page } })">
        Back to Hymns
      </v-btn>
    </div>
        ...
        <v-form v-else @submit.prevent="saveHymn">
          <v-row>
            <v-col cols="12" md="6">
              <v-text-field
                ...
                type="number"
                required
                :rules="[v => !!v || 'Hymn number is required', v => v > 0 || 'Hymn number must be positive']"
          ...
          <v-row>
            <v-col cols="12" md="6">
              <v-select
                ...
                variant="outlined"
                required
                :rules="[v => !!v || 'Hymn category is required']"
          ...
          <v-row>
            <v-col cols="12" class="d-flex justify-end ga-2">
              <v-btn
                variant="outlined"
                @click="$router.push({ path: '/hymns', query: { page: route.query.page } })"
                :disabled="saving"
              >
                Cancel
              </v-btn>
...
async function fetchSongs() {
  try {
    const response = await apiService.getSongs(1, undefined, undefined, undefined, undefined, undefined, undefined, undefined)
...
async function fetchHymn(id: number) {
  ...
  try {
    ...
    error.value = 'Failed to load hymn. ' + (err instanceof Error ? err.message : 'Please try again.')
...
async function saveHymn() {
  ...
  try {
    ...
    router.push({ path: '/hymns', query: { page: route.query.page } })
  } catch (err) {
    error.value = 'Failed to save hymn. ' + (err instanceof Error ? err.message : 'Please try again.')
