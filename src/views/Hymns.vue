<template>
    <v-container fluid class="pa-6 pa-sm-4 pa-xs-2">
        <div
            class="d-flex justify-space-between align-center mb-6 flex-wrap ga-4"
        >
            <div class="flex-grow-1">
                <h1 class="text-h4 font-weight-bold mb-2">Hymns</h1>
                <p
                    class="text-subtitle-1 text-medium-emphasis d-none d-sm-block"
                >
                    Manage hymns
                </p>
            </div>
        ...
        <v-card elevation="2">
            <v-card-title
                class="d-flex justify-space-between align-center bg-surface flex-wrap ga-4"
            >
        ...
        <v-dialog v-model="deleteDialog" max-width="400">
            <v-card>
                ...
                <v-card-text>
                    Are you sure you want to delete hymn
                    <strong>No. {{ selectedHymn?.no }}</strong
...
async function fetchHymns() {
    ...
    try {
        ...
        // If the current page is beyond the total pages, adjust it
        if (page.value > totalPages.value && totalPages.value > 0) {
            page.value = totalPages.value;
            fetchHymns(); // Refetch with the corrected page
            return;
        }
    } catch (err) {
        error.value =
...
function goToSong(songId: number) {
    router.push(`/songs/${songId}/edit?from=hymns&page=${page.value}`);
}
